...
1. Clone the caffe repository
  ```Shell
  git clone https://github.com/PerfXLab/caffe.git
  ```

2. We'll call the directory that you cloned caffe into `CAFFE_ROOT`
...
5. Download pre-computed Faster R-CNN detectors
    ```Shell
    cd $CAFFE_ROOT
    ./example/faster_rcnn/fetch_faster_rcnn_models.sh
    ```

    This will populate the `$CAFFE_ROOT/examples/faster_rcnn` folder with `faster_rcnn_models`.
...
To run the demo
```Shell
cd $CAFFE_ROOT/examples/faster_rcnn
python tool/demo.py
```
...
6. [Optional] If you want to use COCO, please see some notes under `examples/faster_rcnn/README_data.md`
...
Output is written underneath `$CAFFE_ROOT/output`.
...
Output is written underneath `$CAFFE_ROOT/examples/faster_rcnn/output`.
