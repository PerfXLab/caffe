...
import argparse
import logging
sys.path.insert(0,'/home/houxin/project/py-faster-rcnn/lib/')

import numpy as np
caffe_root = '/home/houxin/project/py-faster-rcnn/caffe-fast-rcnn/'
...
def pre_process(expected_proto, new_proto):
    ...
    for idx in range(num_layers):
        ...
        if l.type == "BatchNorm" or l.type == "Scale":
            continue
        elif l.type == "Convolution" or l.type == "Deconvolution" or l.type == "DepthwiseConvolution":
    ...
    for key in listKeys:
        if type(nobn.params[key]) is caffe._caffe.BlobVec:
            ...
            if key not in bn_maps or "bn" not in bn_maps[key]:
                ...
                wt = conv[0].data
                channels = 0
                if bn_maps[key]["type"] == "Convolution" or bn_maps[key]["type"] == "DepthwiseConvolution":
                    channels = wt.shape[0]
