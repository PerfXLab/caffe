...
__global__ void focus_kernel(const Dtype *srcData, const int in_w,
                             const int in_h, const int in_c, const int batch,
  ...
  while (gid < out_elements) {
    ...
    int in_idx = out_b_idx * in_c * in_w * in_h + in_c_idx * in_w * in_h +
...
template <typename Dtype>
void FocusLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype> *> &bottom,
                                    const vector<Blob<Dtype> *> &top) {
  const Dtype *bottom_data = bottom[0]->gpu_data();
  int count = bottom[0]->count();
  Dtype *top_data = top[0]->mutable_gpu_data();
  focus_kernel<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      bottom_data, width_, height_, channels_, batch_num_, stride_, reverse_,
      top_data);
}

template <typename Dtype>
void FocusLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype> *> &top,
...
INSTANTIATE_LAYER_GPU_FUNCS(FocusLayer);
