...
namespace caffe {
  ...
  Dtype delta_region_box(vector<Dtype> truth, Dtype* x, vector<Dtype> biases, int n, int index, int i, int j, int w, int h, Dtype* delta, float scale, int stride) {
    ...
    delta[index + 0 * stride] = (-1) * scale * (tx - sigmoid(x[index + 0 * stride])) * sigmoid(x[index + 0 * stride]) * (1 - sigmoid(x[index + 0 * stride]));
    ...
    for (int b = 0; b < bottom[0]->num(); b++) {
      for (int s = 0; s < side_*side_; s++) {
        for (int n = 0; n < num_; n++) {
          ...
          /*if (iter_ < 12800 / bottom[0]->num()) {
            vector<Dtype> truth;
            truth.clear();
            truth.push_back((x2 + .5) / (float)side_); //center of i,j
